...
public class CMCCDynamicSmsNoticeResource {

    private final static Logger logger = LoggerFactory.getLogger(CMCCDynamicSmsNoticeResource.class);
    ...
    public Response receiveNotice(@FormParam("MchId")String mchId, @FormParam("MchNo")String MchNo, @FormParam("Fee")String Fee, @FormParam("sign") String sign){
        logger.debug("received notice from cmcc dynamic sms,mchno:[{}]", MchNo);
        if(!SignUtil.checkChannelSign(sign, MchNo, mchId, Fee, RechargeConstants.CMCC_DYNAMIC_SMS_APPKEY)){
